...
export default function MissionPage() {
  ...
  return (
    <div className="mx-auto grid min-h-[calc(100svh-120px)] max-w-6xl gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="rounded-[2rem] border border-white/60 bg-white/72 p-5 shadow-[0_24px_90px_rgba(15,23,42,0.10)] backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.05]">
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white/60 p-4 dark:border-white/10 dark:bg-white/[0.03]">
      ...
      <section className="rounded-[2rem] border border-white/60 bg-white/72 p-5 shadow-[0_24px_90px_rgba(15,23,42,0.10)] backdrop-blur-2xl dark:border-white/10 dark:bg-[#07111F]/74">
        {!mission ? (
          ...
          <div className="space-y-5">
            <div className="rounded-3xl border border-slate-200 bg-white/65 p-5 dark:border-white/10 dark:bg-white/[0.03]">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#0E7490]">
                    {formatLabel(mission.mission_type)}
                  </p>
                  <h2 className="mt-2 text-2xl font-semibold text-slate-950 dark:text-white">{mission.objective}</h2>
                  <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{mission.why}</p>
                </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {plan.map((step, index) => (
                <div key={`${step.title}-${index}`} className="rounded-3xl border border-slate-200 bg-white/65 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  ...
                  <p className="mt-3 text-sm leading-6 text-slate-500 dark:text-slate-400">{step.detail}</p>
                </div>
              ))}
            </div>

            {question ? (
              <div className="rounded-3xl border border-slate-200 bg-white/75 p-5 dark:border-white/10 dark:bg-white/[0.04]">
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          !active && !submitted && "border-slate-200 bg-white/70 text-slate-600 hover:border-[#0E7490]/25 hover:bg-[#0E7490]/5 dark:border-white/10 dark:bg-white/[0.03] dark:text-slate-300",
                          correctOption && "border-emerald-400/40 bg-emerald-400/12 text-emerald-700 dark:text-emerald-200",
                          wrongOption && "border-rose-400/40 bg-rose-400/12 text-rose-700 dark:text-rose-200",
                        )}
                      >
                        {option}
                      </button>
            ...
            {report ? (
              <div className="rounded-3xl border border-[#0E7490]/20 bg-[#0E7490]/10 p-5">
                ...
                <h3 className="mt-2 text-xl font-semibold text-slate-950 dark:text-white">{report.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{report.summary}</p>
                <div className="mt-4 space-y-2">
                  {report.next.map((item) => (
                    <div key={item} className="flex gap-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
            ...
            {roadmap.length ? (
              <div className="rounded-3xl border border-slate-200 bg-white/65 p-5 dark:border-white/10 dark:bg-white/[0.03]">
                ...
                <div className="mt-4 space-y-3">
                  {roadmap.map((item) => (
                    <div key={item.condition} className="rounded-2xl border border-slate-200 bg-white/60 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                      <p className="text-sm font-semibold text-slate-900 dark:text-white">{item.condition}</p>
                      <p className="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">{item.next_step}</p>
                    </div>
                  ))}
                </div>
              </div>
